...
static void mbedtls_init(void *instance, const char *host) {
    ...
    mbedtls_ssl_conf_min_version(&result->config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_3);
    ...
    else
    {
        ...
        if (result != NULL)
        {
            ...
            else
            {
                ...
                result->pkey.pk_info =NULL;
                result->pkey.pk_ctx =NULL;
    ...
    else
    {
        ...
        else
        {
            ...
            if (((underlying_io_options = xio_retrieveoptions(tls_io_instance->socket_io)) == NULL) ||
                ...
                result = NULL;
            } else if ((tls_io_instance->x509_ecc_cert != NULL) &&
                ...
                OptionHandler_Destroy(result);
                result = NULL;
            } else if ((tls_io_instance->x509_ecc_aliaskey != NULL) &&
    ...
    else
    {
        ...
        else if (strcmp(OPTION_X509_ECC_KEY, optionName) == 0)
        {
            ...
            else
            {
                ...
                {
                  int parse_result =  mbedtls_pk_parse_key( &tls_io_instance->pkey, (const unsigned char *)value, (int)(strlen(value) + 1), NULL, 0 );
                  if( parse_result != 0 ){
                     LogError("Error mbedtls_pk_parse_key failed\n\n");
                     result = __FAILURE__;
                  } else {
                    parse_result = mbedtls_ssl_conf_own_cert( &tls_io_instance->config, &tls_io_instance->trusted_certificates_parsed, &tls_io_instance->pkey );
                    if( parse_result != 0 ){
                      LogError("Error mbedtls_ssl_conf_own_cert failed\n\n");
                      result = __FAILURE__;
                    } else {
                      LogInfo("x509_ecc_key Initialized\r\n");
                      result = 0;
                    }
                  }
                }
        ...
        else if (strcmp(OPTION_X509_ECC_CERT, optionName) == 0)
        {
          ...
          else
          {
            ...
            {
                LogInfo("x509_ecc_cert Initialized\r\n");
                result = 0;
            }
          }
        }
